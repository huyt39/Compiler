...
#include <stdio.h>
#include <stdlib.h>
// #include <conio.h>  // Not available on macOS/Linux
#include <ctype.h>
#include <string.h>
#include <limits.h>
#include "reader.h"
...
Token* getToken(void)
{
	...
	case 3:
	{
		ln = lineNo;
		cn = colNo;
		int count = 1; // identifier length so far (includes str[0])
		str[0] = (char)currentChar;
		readChar();
		while ((currentChar != EOF) &&
			((charCodes[currentChar] == CHAR_LETTER) || (charCodes[currentChar] == CHAR_DIGIT)))
		{
			// Keep counting full length for "too long" detection,
			// but only store up to MAX_IDENT_LEN characters.
