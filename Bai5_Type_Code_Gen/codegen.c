...
void genVariableAddress(Object* var) {
  // xac dinh muc (level) cua bien
  // hien tai coi tat ca bien o muc 0 (tren frame hien tai)
  int varLevel = 0;

  // xac dinh vi tri offset cua bien
  int varOffset = var->varAttrs->localOffset;

  // sinh ma lenh LA
  genLA(varLevel, varOffset);
}

void genVariableValue(Object* var) {
  // xac dinh muc (level) cua bien va offset cua bien
  // hien tai coi tat ca bien o muc 0
  int varLevel = 0;
  int varOffset = var->varAttrs->localOffset;

  // sinh ma lenh LV
  genLV(varLevel, varOffset);
}
